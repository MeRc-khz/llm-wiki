...
def main():
    parser = argparse.ArgumentParser(description="Transcribe any audio file to LLM Wiki raw/transcripts/")
    parser.add_argument("audio_path", help="Path to the raw audio file")
    parser.add_argument("slug", help="Output filename slug (without extension)")
    ...
    dest_filename = f"{args.slug}{ext}"
    dest_path = os.path.join(RAW_AUDIO_DIR, dest_filename)
    ...
    resolved_title = args.title or args.slug.replace("-", " ").replace("_", " ").title()

    print(f"🎙️ Connecting to Gemini API & uploading {dest_filename} (this may take a few moments)...")
...
source_file: "raw/audio/{dest_filename}"
...
## Metadata
- **Source File:** raw/audio/{dest_filename}
